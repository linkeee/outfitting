...
//        List<SegToken> list = segmenter.process(s.replace(" ", "").replace("\r", "").replace("\n", ""), JiebaSegmenter.SegMode.INDEX);
//        for (SegToken segToken : list) {
//            System.out.println(segToken.word.trim());
//        }
        // 输入目录及输出目录
...
//          System.out.println(entry.getKey().toString() + " == " + value);
    ...
    public static void Rank(HashMap<String, Float> wordmap, String filename) throws IOException {
        BufferedWriter Writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(filename)), StandardCharsets.UTF_8));
